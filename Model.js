        ...
        for (const item in myGameList) {
            ...
            while (i < 1) {
                ...
                if (item == gameListLength - 1) {
                    newPlanet = new Planet(Math.random() * (parseInt(canvas.width) - 200) + 200, Math.random() * parseInt(canvas.height), hoursToSize(currentGame[2]), true, currentGame[1], currentGame[2]);
                } else {
                    if (currentGame[2] >= 2000000) { // LARGE PLANET
                        newPlanet = new Planet(Math.random() * (parseInt(canvas.width) - 200) + 200, Math.random() * parseInt(canvas.height), hoursToSize(currentGame[2]), false, currentGame[1], currentGame[2]);
                        newPlanet.setLarge();
